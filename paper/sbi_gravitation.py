...
class GravitySimulator:
    def __init__(
        ...
        self.m0 = 1.0
        self.m1, self.m2, self.m3, self.v0, self.G, self.dt, self.nsteps = (
            np.array(m1),
            np.array(m2),
            np.array(m3),
            np.array(v0),
        ...
        self.attractor_positions = np.array([[20.0, 20.0], [50.0, -15.0], [80.0, 25.0]])
        # first step special case
        self.positions[1] = (
            self.positions[0]
            + self.v0 * self.dt
            + 0.5 * self.dt**2 * self.A(self.positions[0])
        )
        self.iter_idx = 2
    ...
    def plot_traj(
        ...
        xmin = min(
            x.min(),
            np.min(self.attractor_positions[0, :])
            - 0.1 * abs(np.min(self.attractor_positions[0, :])),
        )
        xmax = max(
            x.max(),
            np.max(self.attractor_positions[0, :])
            + 0.1 * np.max(self.attractor_positions[0, :]),
        )
        plt.xlim(xmin, xmax)
        ymin = min(
            y.min(),
            np.min(self.attractor_positions[1, :])
            - 0.1 * abs(np.min(self.attractor_positions[1, :])),
        )
        ymax = max(
            y.max(),
            np.max(self.attractor_positions[1, :])
            + 0.1 * np.max(self.attractor_positions[1, :]),
        )
        plt.ylim(ymin, ymax)
        axes.scatter(
            self.attractor_positions[:, 0],
